...
def eval_predictor(X_train, Y_train, X_test, Y_test, lam):
    ...
    return np.sqrt(mean_square_loss(X_train, Y_train, th, th0)),np.sqrt(mean_square_loss(X_test, Y_test, th, th0))
...
def xval_learning_alg(X, y, lam, k):
    ...
    for i in range(k):
        ...
        train_value, score_value = eval_predictor(X_train, y_train, X_test, y_test, lam)
        print(train_value, score_value)
